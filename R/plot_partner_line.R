...
#' 
#' This is a function that creates a line graph showcasing volumes by trade partners. This can be aggregated at a regional level.
#' 
...
plot_partner_line <- function(data, trade_flow, prop_flow_cutoff = 0.05, 
  ...
  data %>%
    ggplot() +
    geom_line(aes(x = year, y = quantity, color = partner.name), size = 1.1) +
    scale_color_manual(values = artis_palette(length(unique(data$partner.name)))) +
    labs(y = quantity.lab, x = "Year", title = plot.title, color = partner.lab) +
    theme_bw() 
}
