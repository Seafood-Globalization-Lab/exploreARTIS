#' Creates a line graph of ARTIS data by species
#' 
#' This is a function that creates a line graph showcasing seafood trade volumes by species.
#' 
...
#' @return Line graph of ARTIS data by species.
#' @import tidyverse
#' @import countrycode
#' @import viridis
#' @export

plot_species_line <- function(data, prop_flow_cutoff = 0.05, 
                                 species = NA, years = NA,
  ...
  data %>%
    ...
    labs(y = quantity.lab, x = "Year", title = plot.title, color = "Species/species group") +
    theme_bw() 
}
