#' Creates a line graph traded species from ARTIS data
#' 
...
plot_species_line <- function(data, prop_flow_cutoff = 0.05, 
  ...
  data %>%
    # Plot line graph
    ggplot() +
    geom_line(aes(x = year, y = quantity, color = sciname), size = 1.1) +
    scale_color_manual(values = artis_palette(length(unique(data$sciname)))) +
